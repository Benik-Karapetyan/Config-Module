...
const Tasks: FC<TasksProps> = () => {
  ...
  const toggleTodo = (e: CheckboxChangeEvent, id: string) =>
    dispatch(
      toggleTaskActionCreator({
        id,
  ...
  return (
    <>
      <PageHeader title="Title" subTitle="This is a subtitle" onBack={() => null} />

      <Row align="middle" justify="center">
        <Col span={12}>
          <Card title="My Tasks">
            <Form size="large" ref={formRef} onFinish={onSubmit} onFinishFailed={onError} initialValues={{title: ''}}>
              <Form.Item name="title" rules={[{required: true, message: 'Please input your username!'}]}>
                <Input placeholder="What to do next?" autoComplete="off" ref={inputRef} allowClear bordered />
              </Form.Item>
            </Form>
            ...
            <List
              ...
              renderItem={(item: Task) => (
                ...
                >
                  <List.Item.Meta
                    avatar={
                      <Checkbox onChange={(e) => toggleTodo(e, item.id)} checked={item.done}>
                        {
                          <Text delete={item.done} strong>
                            {item.title}
                          </Text>
                        }
                      </Checkbox>
